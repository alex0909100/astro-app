...
async function drawTarot() {
  ...
  $("#tarot-result").innerHTML = `<div class="tarot-spread"><div class="section-heading"><span class="step">02</span><h2>${escapeHtml(data.spread.title)} · ${escapeHtml(data.topic.title)}</h2></div><p class="tarot-question">«${escapeHtml(data.question)}»</p><div class="tarot-cards">${data.cards.map((card) => `<article class="tarot-card"><div class="tarot-card-inner"><div class="tarot-back">✦<small>ASTRO TAROT</small></div><div class="tarot-front"><span>${card.reversed ? "↺" : "✦"}</span><strong>${escapeHtml(card.name)}</strong><small>${card.reversed ? "ПЕРЕВЁРНУТАЯ" : "ПРЯМАЯ"}</small></div></div><h3>${escapeHtml(card.position)}</h3><p class="muted">${escapeHtml(card.positionPrompt)}</p></article>`).join("")}</div><div class="card tarot-summary"><div class="section-heading"><span class="step">03</span><h2>Интерпретация YandexGPT</h2></div><div class="ai-text">${escapeHtml(data.aiInterpretation)}</div><p class="muted">${data.disclaimer}</p></div>${arcanaAfterSpread}<button class="secondary-button" onclick="shareTarot()">Поделиться раскладом ↗</button></div>`;
}

async function loadAiInterpretation(kind, selector = "#ai-chart") {
  const target = $(selector); if (!target) return;
  try {
    const response = await apiFetch("/api/interpretation", {method:"POST", body:JSON.stringify({chart:state.chart, kind})});
    const data = await response.json();
    target.innerHTML = `<div class="section-heading"><span class="step">YANDEXGPT</span><h2>Персональный разбор</h2></div><div class="ai-text">${escapeHtml(data.text || "Разбор временно недоступен.")}</div>`;
  } catch (error) {
    target.innerHTML = `<div class="section-heading"><span class="step">РАЗБОР</span><h2>Временно недоступен</h2></div><p class="muted">Основная карта уже готова. Попробуйте обновить раздел позже.</p>`;
  }
}
...
async function loadForecast(period = "day") { const response = await apiFetch(`/api/forecast/${period}?userId=${encodeURIComponent(state.userId)}`); const data = await response.json(); if (!response.ok) { $("#forecast").innerHTML = `<div class="card forecast"><p>${escapeHtml(data.error || "Прогноз временно недоступен.")}</p></div>`; return; } $("#forecast").innerHTML = `<div class="card forecast"><p class="eyebrow">ПРОГНОЗ · ${period.toUpperCase()} · УДАЧА ${data.luck}/10</p><p class="ai-text">${escapeHtml(data.text)}</p><small>${data.disclaimer}</small></div>`; }
