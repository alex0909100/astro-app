...
async function drawTarot() {
  ...
  $("#tarot-result").innerHTML = `<div class="tarot-spread"><div class="section-heading"><span class="step">02</span><h2>${escapeHtml(data.spread.title)} · ${escapeHtml(data.topic.title)}</h2></div><p class="tarot-question">«${escapeHtml(data.question)}»</p>${data.personalArcana ? `<div class="arcana-strip">Аркан судьбы: ${escapeHtml(data.personalArcana.destiny.name)} · ${escapeHtml(data.personalArcana.destiny.light)}</div>` : ""}<div class="tarot-cards">${data.cards.map((card) => `<article class="tarot-card"><div class="tarot-card-inner"><div class="tarot-back">✦<small>ASTRO TAROT</small></div><div class="tarot-front"><span>${card.reversed ? "↺" : "✦"}</span><strong>${escapeHtml(card.name)}</strong><small>${card.reversed ? "ПЕРЕВЁРНУТАЯ" : "ПРЯМАЯ"}</small></div></div><h3>${escapeHtml(card.position)}</h3><p class="muted">${escapeHtml(card.positionPrompt)}</p><p>${escapeHtml(card.interpretation)}</p></article>`).join("")}</div><div class="card tarot-summary"><div class="section-heading"><span class="step">03</span><h2>Общий итог</h2></div><p>${escapeHtml(data.summary)}</p><p class="muted">${data.disclaimer}</p></div><button class="secondary-button" onclick="shareTarot()">Поделиться раскладом ↗</button></div>`;
}

function renderTarotArcana() {
