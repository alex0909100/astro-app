...
async function loadDailyForecast() {
  ...
  try {
    ...
    target.innerHTML = `<div class="section-heading"><span class="step">СИГНАЛЫ КОСМОСА</span><h2>Ваш прогноз на сегодня</h2></div><div class="ai-text">${escapeHtml(data.text)}</div><p class="muted">Прогноз обновляется каждый день.</p>`;
...
async function loadAiInterpretation(kind, selector = "#ai-chart") {
  ...
  try {
    ...
    const data = await response.json();
    target.innerHTML = `<div class="section-heading"><span class="step">ПОСЛАНИЕ КОСМОСА</span><h2>Персональный разбор</h2></div><div class="ai-text">${escapeHtml(data.text || "Разбор временно недоступен.")}</div>`;
  } catch (error) {
